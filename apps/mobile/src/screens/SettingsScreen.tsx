    ...
    <View style={styles.root}>
      ...
      <View style={styles.row}>
        {(["dark", "light", "auto"] as const).map((t) => (
          <TouchableOpacity key={t} style={[styles.chip, settings.theme === t && styles.chipOn]} onPress={() => updateSettings({ theme: t })}>
      ...
      <View style={styles.row}>
        {(["balanced", "concise", "detailed"] as const).map((s) => (
          <TouchableOpacity key={s} style={[styles.chip, settings.responseStyle === s && styles.chipOn]} onPress={() => updateSettings({ responseStyle: s })}>
...
const styles = StyleSheet.create({
  ...
  chipOn: { borderColor: colors.accent, backgroundColor: "#8b5cf622" },
